...
function Editor() {
  ...
  const handleSend = useCallback(async () => {
    ...
    let content = getFeedbackContent()?.trim()
    if (!content) return
    useDatasetStore
      .getState()
      .datasets.sort((a, b) => b.name.length - a.name.length)
      .forEach((dataset) => {
        content = content
          .split(`@${dataset.name}`)
          .join(`@[${dataset.name}](ds-${dataset.id})`)
      })
    try {
      clearFeedbackContent()
  ...
  const handleSubmit = useCallback(
    ...
    [handleSend, inputState, loadingPanelMessages, feedbackChatId],
  )
...
export function FeedbackChatInput() {
  return (
    <LexicalComposer initialConfig={feedbackEditorConfig}>
      <div className='mx-3 flex w-full min-w-[15rem] max-w-full flex-col items-center rounded-lg border border-border bg-white p-2'>
        <Editor />
        <HistoryPlugin />
      </div>
    </LexicalComposer>
